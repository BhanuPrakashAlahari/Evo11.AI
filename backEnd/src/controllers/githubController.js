...
export async function redirectToGithubAuth(req, res, next) {
  try {
    ...
    // Determine clientUrl dynamically: prioritize req.query.redirect_origin, then config.clientUrl, then fallback
    let clientUrl = req.query.redirect_origin || config.clientUrl || 'http://localhost:5173'
